...
### Round 16 — Prototype: rotating card carousel สำหรับ dashboard (task card "เปิดคิวตรวจ") (2026-09-18)

**บริบท:** ผู้ใช้ขอ component ใหม่ — การ์ดหมุนเวียน 3 ใบแสดงงาน KAIZEN ที่รอตรวจ (ชื่อผู้ส่ง/แผนก/โรงงาน/รูปแนบ 1 รูป) อิงเทคนิค CSS จาก Uiverse.io (ผู้เขียนต้นฉบับ: ilkhoeri) ทำเป็น Claude Artifact แยกต่างหาก **ไม่ใช่โค้ดใน repo จริง** — เป็นขั้นตอน prototype/design exploration ก่อนตัดสินใจว่าจะเอาไปต่อยอดใน `dashboard.js` จริงหรือไม่

**กระบวนการ:** ทำหลายรอบตามฟีดแบ็กละเอียดของผู้ใช้ — เริ่มจากเนื้อหาทั่วไป (travel destination) → ปรับเป็นเนื้อหา KAIZEN จริง (ตัดดาว/รีวิวออก เหลือแค่ผู้ส่ง+แผนก+โรงงาน+รูป) → ผู้ใช้ส่งซอร์สโค้ดต้นฉบับ Uiverse เต็มมาเทียบ ถามชัดเจนแล้วว่าต้องการรวมกลไก/ลูกเล่นจากต้นฉบับ**เข้ากับ**เวอร์ชัน KAIZEN เดิม ไม่ใช่ revert กลับไปเนื้อหาเดิม → ปรับสี/token ให้ตรงกับแอปจริง (ดึงจาก `css/style.css`: `--primary:#0066cc` ฯลฯ) และปุ่มใช้ label จริงจาก i18n key `dashboard_open_review_btn` ("เปิดคิวตรวจ") → ปรับตำแหน่ง/ขนาดเส้นเรืองแสงใต้การ์ดและปุ่ม CTA หลายรอบตาม pixel-level feedback

**บั๊กที่เจอระหว่างทำ (ผู้ใช้วินิจฉัยเองก่อนบางส่วน):**
- การ์ดโดนตัดขอบล่างเพราะ `.wrap_card` สูงไม่พอ — ผู้ใช้ชี้ถูกว่า container เตี้ยกว่าการ์ดจริง แต่แก้ตามที่ผู้ใช้เสนอ (`height: var(--card-h)` ตรงๆ) ยังไม่พอ ต้องคำนวณเพิ่มอีกชั้น: การหมุน `rotate(15deg)` ทำให้ bounding box ของกล่องไม่เป็นสี่เหลี่ยมจัตุรัสโตขึ้นจริง (`W·sin(θ)+H·cos(θ)`) รวมกับระยะเลื่อน `--shift-y` ต้องบวกเผื่อรวม ~40px จากค่าตั้งต้น
- เอฟเฟกต์ mask-image แบบ radial-gradient ที่ใช้บังเส้นเรืองแสงมีจุดบอด (มุมกล่องที่หมุนแล้วโผล่ทะลุที่ขอบ) เปลี่ยนไปใช้วิธี z-index stacking ธรรมดาแทน ทนกว่า

**สถานะ:** ผู้ใช้บอกพักไว้แค่นี้ก่อน จะทำต่อเอง — เซฟไฟล์ไว้ที่ `prototypes/kaizen-deck.html` (ย้ายจาก scratch folder ชั่วคราวเดิม) **ยังไม่ commit เข้า git**, ยังไม่ได้เอาไปต่อกับ `dashboard.js` จริง เป็นแค่ไฟล์ตัวอย่างสแตนด์อโลนสำหรับดูงานออกแบบ Artifact link (เผื่อใช้อ้างอิง แชร์ทีมได้): `https://claude.ai/code/artifact/fc9f483b-64ef-4e82-ad41-709489515a58`

**ไฟล์ที่เปลี่ยน:** เฉพาะ `prototypes/kaizen-deck.html` (ไฟล์ใหม่ ยังไม่ track ใน git) — ไม่กระทบโค้ด production ใดๆ

---

### Round 17 — เอา rotating card deck เข้า production จริง + ปรับ UI ย่อยหลายจุด (2026-09-18)

**บริบท:** ต่อจาก Round 16 (prototype แยกต่างหาก) ผู้ใช้ทดสอบผ่าน localhost เอง ("สร้างใน dashboard เลย แทนที่อันนี้") สั่งเอาการ์ดหมุนเข้ามาแทนที่แถวปุ่มธรรมดาของงาน "หน้าที่กรรมการ" ใน `dashboard.js` จริง

**1) Data layer ใหม่ที่ต้องเพิ่ม (ของเดิมไม่มี):**
- `getReviewQueue()` (`js/api.js`) เดิม select แค่ `kaizen_attachments(id)` (นับจำนวนรูปอย่างเดียว) ขยายเป็น `kaizen_attachments(*)` (เอา `storage_path` มาทำ signed URL รูปปก) + เพิ่ม owner name — join แบบแยกคำขอ (pattern เดียวกับ `getFeedPage()` เดิม เพราะ `kaizen_projects` มี FK ไป `profiles` สองเส้น `owner_id`/`responsible_user_id` ทำให้ PostgREST embed ตรงๆ กำกวม) ยืนยันจากอ่าน RLS จริงว่า policy `profiles_read_related` (ผ่าน `can_read_kaizen()`) อนุญาตกรรมการอ่านชื่อเจ้าของโครงการ `pending_review` ในรอบตัวเองอยู่แล้ว ไม่ต้องแก้ policy ใดๆ — ทดสอบสดยืนยันว่า join ทำงานจริง ไม่โดน RLS บล็อก

**2) ปัญหาจริงที่ค้นพบก่อนเริ่มโค้ด: prototype ออกแบบมาสำหรับ 3 ใบตายตัว แต่ dashboard จริงตอนทดสอบมีแค่ 1 โครงการรอตรวจ** — ถามผู้ใช้ 3 คำถามก่อนเริ่ม (ปรับ layout ตามจำนวนจริง / ใช้รูปแนบจริงไหม / ดึงชื่อผู้ส่งจากไหน) ผู้ใช้เลือกตัวเลือกแนะนำทั้ง 3 ข้อ ได้กลไก 3 แบบตามจำนวนโครงการ (`.deck-wrap.is-count-1/2/3`):
- 1 ใบ: การ์ดนิ่งไม่หมุน
- 2 ใบ: สลับหน้า-หลังกันโดยใช้ keyframe ร่วมชุดเดียวกับ 3 ใบ (สัดส่วนเวลาหน้า/หลังไม่เท่ากันเป๊ะ 50/50 แต่ยอมรับได้ เพราะเกิดไม่บ่อยเท่า 3 ใบ ไม่คุ้มแยก keyframe ใหม่)
- 3 ใบขึ้นไป: หมุนเวียนแบบเดียวกับ prototype เป๊ะ (โชว์ 3 โครงการล่าสุดตาม `SubmittedAt`)

**3) Mobile responsive** — ขนาดการ์ดเดิมคงที่ 150px (มาจาก prototype, ออกแบบไว้บนจอ desktop) ล้นจอมือถือแคบ ทำ `clamp()` ผูกกับ `vw` แทนค่าคงที่ ให้โตตามจอจริง (110px ที่จอ 320px ไล่ไปจนสุดที่ 150px ที่จอ ~417px ขึ้นไป) ทดสอบยืนยันด้วย `emulate` ที่ 320/375/430/759px ไม่มีล้น ไม่มีตัดขอบ

**4) บั๊กจริงที่ใช้เวลาสืบเยอะที่สุดในรอบนี้ — ไม่ใช่บั๊กเบราว์เซอร์ เป็นบั๊กจากคอมเมนต์ตัวเอง:** เพิ่มกฎ `.task-row.is-deck { flex-direction: column; ... }` แล้ว layout ไม่เปลี่ยนเลยทั้งที่โค้ดถูกต้องทุกอย่างและ Console ไม่มี error ใดๆ ไล่ด้วยการ isolate CSS ผ่าน `CSSStyleSheet.replaceSync()` จนเจอสาเหตุจริง: คอมเมนต์ CSS ก่อนหน้ากฎนั้นมีข้อความอธิบาย custom property `--deck-z*/--deck-t*` ซึ่งบังเอิญมีตัวอักษร `*/` แฝงอยู่ (จาก `z*` ต่อด้วย `/`) — CSS ปิดคอมเมนต์ที่ `*/` ตัวแรกเสมอไม่สนบริบท ทำให้เนื้อหาที่เหลือหลุดเป็น CSS ดิบผิด syntax เบราว์เซอร์เลยข้ามกฎถัดไปทั้งก้อนไปเงียบๆ จนกว่าจะเจอ `*/` ตัวจริงทีหลัง แก้โดยเปลี่ยนคำอธิบายเป็น `--deck-z1..3, --deck-t1..3` (ไม่มี `*/` แฝง) แล้วเช็ค `/*`-`*/` ทั้งไฟล์ให้ครบคู่ทุกครั้งหลังจากนี้ก่อน publish

**5) บั๊ก stacking context ซ้ำ 2 รอบ (บทเรียนเดียวกัน คนละจุด):** ทั้ง `align-self` และ `z-index` (ตอนขยับปุ่ม "เปิดคิวตรวจ" ขึ้นไปทับพื้นที่การ์ด) ใส่ผิดที่ตั้งแต่ครั้งแรก — ใส่ที่ `.deck-cta` (`<button>`) แทนที่จะเป็น `<a>` ที่ห่ออยู่ (ตัวที่เป็น flex item จริงของ `.task-row.is-deck`) ทำให้ไม่มีผลอะไรเลยเพราะ `<button>` ไม่ใช่ตัวที่แข่ง stacking/alignment กับ `.deck-wrap` ที่เป็นพี่น้องกัน แก้โดยย้ายทั้งคู่ไปที่ `<a>` แทน

**6) ปรับแต่งย่อยตามฟีดแบ็กพิกเซล/สีหลายรอบ:** กล่องขาวใต้เส้น (สูง 60→80px), ปุ่มขยับตำแหน่งหลายครั้ง (สุดท้าย margin-top ยกขึ้นรวม 100px จาก baseline), สีกรอบการ์ด/เส้นใต้เปลี่ยนจาก primary/info/violet เป็นฟ้าเรืองแสงเดียวกับ `.welcome-blob` ก่อน แล้วเปลี่ยนกรอบเป็นดำ+ตัด glow ออกทีหลัง, เส้นใต้เพิ่ม blur 4→10px และเปลี่ยนเป็นสีน้ำเงินเข้มกว่าการ์ด, ลดความหนากรอบการ์ด 2px→1px, เอากล่องขาวรอบการ์ดทั้งแถวออกให้กลืนพื้นหลังหน้า

**7) ฟีเจอร์เพิ่มนอกเรื่องการ์ดหมุน (ผู้ใช้ขอต่อเนื่องระหว่างทดสอบ):**
- การ์ดทักทายใหม่เหนือการ์ดรอบประเมิน ("สวัสดี" / ชื่อผู้ใช้ คนละบรรทัด + avatar ซ้ายมือ) — reuse i18n key `dashboard_welcome` ที่นิยามไว้แล้วแต่ไม่เคยถูกเรียกใช้จริงมาก่อน (เจอจาก i18n coverage audit เดิม)
- Sidebar/mobile-drawer avatar เปลี่ยนจากรูป/ตัวอักษรย่อเป็นไอคอน `ICONS.more` (เส้น 3 ขีด, ของเดิมที่มีอยู่แล้วในระบบ) — รอบแรกลืมเอา wrapper วงกลมสีเดิมออก ทำให้สี/ขนาดไม่ตรงกับไอคอนอื่นในแถบ ต้องแก้อีกรอบให้ไม่มี background/border-radius เลย เข้าชุดกับ `.nav-icon`/`.tabbar-icon` จริง — สุดท้ายผู้ใช้ขอแยกกัน: **desktop sidebar ใช้ไอคอน, mobile drawer กลับไปใช้รูปจริงเหมือนเดิม** (`userBoxHtml()` เพิ่ม parameter `showPhoto`ควบคุม) ยืนยันแล้วว่าไม่กระทบ iPad เพราะทุกรุ่น/ทุกแนวกว้างเกิน breakpoint มือถือ 760px เสมอ (เห็น desktop sidebar เท่านั้น)

**ทดสอบสด:** ทุกจุดในรอบนี้ทดสอบผ่านบัญชี test1 (กรรมการ) จริงบน localhost ทุกครั้งก่อน/หลังแก้ (ไม่ใช่แค่อ่านโค้ด) รวมถึงให้ผู้ใช้เองสร้าง KAIZEN จริงเพิ่ม 2 เรื่องระหว่างทางเพื่อทดสอบ N=3 กรณีจริง (ไม่ใช่ mock data)

**ไฟล์ที่เปลี่ยน:** `js/api.js` (`getReviewQueue`), `js/views/dashboard.js` (deck integration + การ์ดทักทาย), `css/style.css` (deck styles + sidebar icon), `js/app.js` (`userBoxHtml`, sidebar/tabbar icon), `index.html` (version bump หลายรอบ: `style.css` z37→z55, `app.js` n→t) — bump shared tag `js/*`/`js/views/*` ทุกไฟล์ `z10`→`z11` ตาม `getReviewQueue` ที่แก้ — `prototypes/kaizen-deck.html` ยังคงอยู่เป็น reference เดิม ไม่ได้ลบ (โค้ดจริงใน `dashboard.js` แยกกันคนละชุด ไม่ได้ import ไฟล์นี้)

---

### Round 18 — น้ำหนักกรรมการอัตโนมัติ + หน้าสร้างรอบใหม่แบบหน้าเดียวจบ (2026-09-18)

**โจทย์จากผู้ใช้:** "การเปิดรอบโครงการใหม่ กับการเลือกกรรมการพร้อมน้ำหนักคะแนน มีวิธีที่ง่ายกว่านี้ไหม" → ตามด้วย "ไม่อยากทำหลายขั้นตอน อยากเปิดรอบแล้วเสร็จเลย"

**1) ค้นพบว่า "ค่ามาตรฐาน" มีอยู่ในระบบตั้งแต่แรกแต่ไม่เคยถูกใช้:** `master_data` type `committee_role` มีฟิลด์ `extra.default_weight_pct` ตั้งค่าไว้ตั้งแต่ `seed.sql` (Production Director 30 / Asst Director 20 / Plant manager 30 / HR 10 / MT-UT 10 = 100 พอดี) และหน้า `adminMaster.js` ก็มีช่องให้แอดมินแก้อยู่แล้ว — **แต่ `adminPeriodDetail.js` ไม่เคยอ่านค่านี้เลย** แอดมินต้องพิมพ์ % เองทีละคนให้ครบ 100 ทุกครั้ง (3 คนต้องคิดเอง 33.33) ตารางน้ำหนักจริงที่ผู้ใช้ส่งมา (ไฟล์ xlsx บน Desktop ของผู้ใช้เอง ไม่ได้อยู่ใน repo) ตรงกับค่าที่ seed ไว้ 100% ต่างแค่ label EN 2 ตัวซึ่งมีคนอัปเดตใน DB จริงไปก่อนหน้านี้แล้ว

**2) บั๊กที่เจอระหว่างทาง:** `getCommitteeCandidates()` ไม่เคย `select` คอลัมน์ `committee_role` มาเลย ทำให้ `candidateInfo()`'s `c.CommitteeRole` เป็น `undefined` เสมอ (ช่องตำแหน่งใต้ชื่อกรรมการว่างเปล่ามาตลอด) และเป็นเหตุผลว่าทำไมทำ auto-fill ตาม role ไม่ได้ถ้าไม่แก้ตรงนี้ก่อน

**3) กลไกน้ำหนักมาตรฐาน** — `computeStandardWeights()` จัดกลุ่มกรรมการที่ active ตาม `committee_role` แล้วหาร `default_weight_pct` ของ role นั้นเท่าๆ กันตามจำนวนคนที่ถือ role เดียวกัน (Plant manager 3 คน → คนละ 10) ใช้ 2 ทาง:
- **อัตโนมัติ** ตอนเปิดหน้ารอบที่เป็น `draft` และยังไม่มีน้ำหนักใน DB เลย
- **ปุ่ม "ใช้ค่ามาตรฐาน (ตามตำแหน่ง)"** กดคำนวณใหม่ทั้งชุดเมื่อไหร่ก็ได้ (คนที่ยังไม่ได้ตั้ง `committee_role` จะถูกข้าม ต้องเพิ่มเอง)

**4) หน้าสร้างรอบใหม่แบบหน้าเดียว (`#/admin/periods/new`, ไฟล์ใหม่ `adminPeriodNew.js`):** เดิมฟอร์มสร้างรอบฝังอยู่ล่างสุดของหน้า list สร้างเสร็จเด้งไปหน้า detail เพื่อตั้งน้ำหนักอีกที — ผู้ใช้ขอให้รวมเป็นหน้าเดียว ตอนนี้หน้า list เหลือแค่ปุ่ม "+ สร้างรอบใหม่" มุมขวาบน กดแล้วเข้าหน้าใหม่ที่มีข้อมูลรอบ (ซ้าย) + น้ำหนักกรรมการที่เติมให้อัตโนมัติ (ขวา) แล้วเลือกได้ 2 ทาง: **"สร้างและเปิดรอบทันที"** (เปิดได้เฉพาะตอนน้ำหนักรวม 100%) หรือ **"สร้างรอบเป็นร่าง"** — `createPeriod()` ส่ง `CommitteeWeights` ไปพร้อมกันในคำขอเดียว (เป็น jsonb บนตารางเดียวกัน ไม่ต้อง insert แล้ว update แยก)

**5) โมดูลร่วม `js/committeeWeights.js` (ไฟล์ใหม่):** ย้ายโค้ดคำนวณ/แสดงผล/ผูก event ของกล่องน้ำหนักออกจาก `adminPeriodDetail.js` มาไว้ที่เดียว ใช้ร่วมกับหน้าสร้างรอบใหม่ — กันโค้ด 2 ชุดที่แก้ทีต้องแก้ 2 ที่แล้วหลุดไม่ตรงกัน (แยกเป็น `weightHeaderHtml` / `weightBodyHtml` เพราะหน้า detail ต้องเอาหัวข้อไว้นอกกล่องที่ยุบได้)

**6) กล่องน้ำหนักในหน้า detail ยุบไว้ก่อน (ผู้ใช้ขอ):** ปกติน้ำหนักถูกตั้งครบตั้งแต่ตอนสร้างรอบแล้ว คนที่เข้าหน้านี้ส่วนใหญ่มาดูยอดส่ง/ตัดสินโครงการ — เห็นแค่หัวข้อ + badge รวม% + ปุ่ม "แก้ไขน้ำหนัก" (รอบที่ปิดแล้วเป็น "ดูน้ำหนัก") กดค่อยคลี่ออกมาแบบ slide (`.collapse-body` ใน `style.css` + เคารพ `prefers-reduced-motion`) — **ข้อยกเว้น:** รอบร่างที่ระบบเพิ่งเติมน้ำหนักให้เอง (ยังไม่บันทึกลง DB) จะกางไว้ตั้งแต่แรก ไม่ควรซ่อนค่าที่แอดมินยังไม่เคยเห็น

**7) บั๊ก UX เดิมที่เจอตอนทดสอบแล้วแก้ไปด้วย:** แก้ตัวเลข % ในช่องแล้ว badge "รวม x%" / checklist / ปุ่มเปิดรอบ ไม่อัปเดตจนกว่าจะมี re-render จากเรื่องอื่น (กดบันทึก/เพิ่ม/ลบคน) — บนหน้าสร้างรอบใหม่อาการหนักกว่าเพราะปุ่ม "สร้างและเปิดรอบ" ค้าง disabled ทั้งที่แก้ให้ครบ 100 แล้ว แก้โดยเพิ่ม listener `change` (ตอนออกจากช่อง/กด Enter) ให้ rerender — ไม่ผูกกับ `input` เพราะจะ re-render ทุกตัวอักษรจน cursor หลุด

**เรื่อง route ที่ต้องระวัง:** `matchRoute()` ใน `router.js` คืน route แรกที่ match และ `:id` จับได้ทุกค่ารวมถึงคำว่า `new` (segment เท่ากัน) — `#/admin/periods/new` จึงต้องประกาศ**ก่อน** `#/admin/periods/:id` เสมอ ถ้าสลับลำดับจะเปิดหน้า detail โดยส่ง id = "new" (มีคอมเมนต์เตือนไว้ในไฟล์แล้ว)

**ทดสอบสด (บัญชี admin จริง, localhost):** สร้างรอบทดสอบจริงผ่านหน้าใหม่ → น้ำหนักบันทึกลง DB พร้อมตอนสร้าง ✅ / เส้นทาง error ตอนกด "สร้างและเปิดรอบ" แล้ว `open_period()` ไม่ผ่าน (มีรอบอื่นเปิดค้างอยู่) → โชว์ error ภาษาไทย ไม่สร้างรอบซ้ำ ล็อกฟอร์ม โชว์ปุ่มเปิดซ้ำ ✅ / หน้า detail หลัง refactor ทั้งรอบร่างและรอบเปิดจริง (ข้อมูล production) ปุ่มใช้ค่ามาตรฐาน-แก้ค่า-ลบคน ทำงานครบ ✅ / ยุบ-กาง + คงสถานะกางหลัง re-render ✅ / มือถือ 390px ไม่มี overflow ✅ — ลบรอบทดสอบทิ้งและคืนค่า `committee_role` ของบัญชีทดสอบกลับหมดแล้ว

**ยังไม่ได้ทำ (ตามที่ผู้ใช้เลือก):** ยังไม่สร้างบัญชีจริงให้กรรมการ 7 คนตามตารางจริง — กลไกพร้อมแล้ว แต่ auto-fill จะให้ผลเต็ม 100% ต่อเมื่อคนเหล่านั้นมีบัญชี + ตั้ง `committee_role` ครบ (ตอนนี้ระบบมีแต่บัญชีทดสอบ)

**ไฟล์ที่เปลี่ยน:** ใหม่ `js/committeeWeights.js`, `js/views/adminPeriodNew.js` — แก้ `js/api.js` (`getCommitteeCandidates` เพิ่ม `committee_role`), `js/router.js` (route ใหม่), `js/views/adminPeriods.js` (ตัดฟอร์มออก เหลือปุ่ม), `js/views/adminPeriodDetail.js` (ใช้โมดูลร่วม + ยุบได้), `css/style.css` (`.collapse-body`), `js/i18n.js` (8 คีย์ใหม่ TH/EN) — bump shared tag `z11`→`z15`, `style.css` z55→z56, `app.js` t→w

...
import { getQuery, ROUTES } from '../router.js?v=20260911z7';
import { escapeHtml, pageHeader, stateCard, roleLabel } from '../ui.js?v=20260911z7';
  ...
  container.innerHTML = `
    ...
    <div class="page-body">
      ${stateCard({
        ...
        body: `
          ${needed.length > 0 ? `ต้องมีสิทธิ์: <strong>${needed.map((r) => escapeHtml(roleLabel(r))).join(', ')}</strong><br/>` : ''}
          สิทธิ์ที่คุณมีตอนนี้: <strong>${have.length > 0 ? have.map((r) => escapeHtml(roleLabel(r))).join(', ') : 'ไม่มี'}</strong><br/>
          ถ้าควรมีสิทธิ์นี้ แจ้งผู้ดูแลระบบพร้อมรหัสพนักงาน <span class="mono">${escapeHtml(session?.profile?.EmployeeId ?? '—')}</span>
        `,
